...
HOOK(end_master_display_list, AT(FUNCTION_CALL), TRUE, 0)
void display_profiler(bool* cancel) {
    ...
    gDisplayListHead--;
    index = D_800DC668^1;
    deltaTime = gProfilerFrameData[index].gameTimes[AFTER_DISPLAY_LISTS] - gProfilerFrameData[index].gameTimes[THREAD5_END];
